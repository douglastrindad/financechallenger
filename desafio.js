...
const resultAll = document.querySelector(".resultado");
...
const RenderTable = (dados) => {
  ...
  dados.forEach((dados) => {
    html += `
      <tr>
        <td>${new Date(dados.date).toLocaleDateString("pt-BR", {timeZone: "UTC",})}</td>
        <td>${dados.customer.first_name} ${dados.customer.last_name}  </td>
        <td>${dados.customer.phone}</td>
        <td> ${dados.store.name}  </td>
        <td> ${dados.store.phone}  </td>
        <td> ${dados.type === "IN" ? "Entrada" : "Saída"} </td>
        <td> ${dados.amount} </td>
        <td>${dados.price.toLocaleString("pt-br", {style: "currency", currency: "BRL",})}</td>
        <td>${convertPrice(dados.amount * dados.price)}</td>
      </tr>
  `;
  });
...
const RenderSelect = () => {
  ...
  dados.forEach((dados) => {
    if (types.includes(dados.type)) {
      return;
    }

    types.push(dados.type);

    html += `
      <option value="${dados.type}">${dados.type === "IN" ? "Entrada" : "Saída"}</option>
    `;
  });
...
const filterSelect = () => {
  const typeFilter = dados.filter((dados) => {
    if (!select.value) {
      return true;
    }

    return select.value === dados.type;
  });

  RenderTable(typeFilter);
  result(typeFilter);
};
...
let Out = 0;
  ...
  value.forEach((dados) => {
    if (dados.type === "IN") {
      In = In + dados.price * dados.amount;

    }

    if (dados.type === "OUT") {
      Out = Out - dados.price * dados.amount;
  ...
  html += ` <label class="resultado" style="color: orange;" }>Total de entradas:${convertPrice(In)}</label>
          <label class="resultado" style="color: red;"> Total de saídas:${convertPrice(Out)} </label>
          <label class="resultado" style="color: blue;"> Saldo:${convertPrice(In + Out)} </label>`;
